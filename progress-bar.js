...
class ProgressBar {
    constructor(total, opts) {
        ...
        this.options = Object.assign({
            barsize: 20,
            format: '{lines} ['+colors.cyan('{bar}')+'] {percentage}% | ETA: {eta}s | {value}/{total}',
            completeChar: '\u2588',
            incompleteChar: '\u2591'
        }, opts);
        this.options.completeBar = this.options.completeChar.repeat(this.options.barsize);
        this.options.incompleteBar = this.options.incompleteChar.repeat(this.options.barsize);
    ...
    drawBar(val, opts) {
        ...
        const context = Object.assign({
            ...
            bar: options.completeBar.substr(0,len)+options.incompleteBar.substr(len, options.barsize),
            value: this.value,
            total: this.total,
            eta: 0==this.value?'-':Math.ceil(this.total/this.value*duration-duration),
            duration: duration,
            desc: ''
        }, opts);
