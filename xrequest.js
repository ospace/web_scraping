...
const barOptions = {
  format:
    "{prefix} [" +
    chalk.cyan("{bar}") +
    "] {percentage}% | ETA: {eta}s | {value}/{total} | {desc}",
};
...
function Request(url, options) {
  ...
  if (options.contentType) options["Content-Type"] = options.contentType;
...
function requestBar(request) {
  ...
  request.on("response", (response) => {
    ...
    if ("chunked" === transEncode) {
      ...
      bar = new progressbar(
        contentLength,
        Object.assign({ prefix: "FILE" }, barOptions)
      );
    }
...
function requestEvent(request) {
  ...
  request
    .on("response", (response) => {
      ...
      response
        .on("data", (chunk) => event.emit("data", chunk))
        .on("error", (err) => {
